...
The aspects and attributes of weather conditions included in the scope of the NETN METOC module are based on input from several sources and designed to cover the most common levels of representation required by a broad set of existing simulators.
...
### Weather
The `Weather` object class represents typical weather-related attributes. Additional environment condition details for `LandSurface`, `WaterSurface` and in `TroposphereLayer` are provided in corresponding subclasses.
...
Besides the common attributes such as Temperature, Wind and Precipitaion etc. additional detail regarding snow, moisture, and ice conditions of the land surface can be represented using the `LandSurface` object class. All attributes of LandSurfaceCondition objects are optional. 
...
Condition of sea surface in a specified region is represented using the `WaterSurface` object class. An instance of this class without a `GeoReference` is considered to apply to all water surfaces.
...
The `TroposphereLayer` object class is used to associate weather and environment contitions to a specific layer in the troposphere. Additional information on cloud coverage can be defined.
...
Environment condition in a subsurface water layer is represented using a `SubsurfaceLayer` object class. If `GeoReference` is not provided, the condition is considered global within the specified layer.
...
For a different type of requests use the corresponding `Request` interaction subclasses. Requests for subsurface and troposphere data include parameter for specifying the related layer information.
